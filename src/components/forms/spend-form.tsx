...
// Define proper type for audit result
...
export default function SpendForm() {
  // Lazy initialization from localStorage to avoid cascading renders
  const [tool, setTool] = useState(() => localStorage.getItem("tool") || "ChatGPT");
  const [monthlySpend, setMonthlySpend] = useState(() => localStorage.getItem("monthlySpend") || "");
  ...
  return (
    <div className="bg-[#0f172a] border border-white/10 rounded-[32px] p-8 md:p-10 shadow-2xl">
      ...
      <div className="grid md:grid-cols-2 gap-6">
        <div>
          <label className="block mb-3 text-sm text-slate-300">AI Tool</label>
          <select
            value={tool}
        ...
        <div>
          <label className="block mb-3 text-sm text-slate-300">Monthly Spend ($)</label>
          <input
            type="number"
            value={monthlySpend}
        ...
        <div>
          <label className="block mb-3 text-sm text-slate-300">Team Size</label>
          <input
            type="number"
            value={teamSize}
      ...
      {result && (
        <div className="mt-8 bg-blue-500/10 border border-blue-500/20 rounded-2xl p-6">
          <h3 className="text-2xl font-bold mb-4">Audit Results</h3>
          <div className="space-y-4">
            <div>
              <p className="text-slate-400 text-sm">Recommendation</p>
              <h4 className="text-xl font-semibold">{result.recommendation}</h4>
            </div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="bg-[#020617] rounded-2xl p-5 border border-white/10">
                ...
                <h4 className="text-3xl font-bold text-green-400 mt-2">${result.savings}</h4>
              </div>

              <div className="bg-[#020617] rounded-2xl p-5 border border-white/10">
                <p className="text-slate-400 text-sm">Annual Savings</p>
                <h4 className="text-3xl font-bold text-blue-400 mt-2">${result.annualSavings}</h4>
              </div>
            </div>

            <div className="bg-[#020617] rounded-2xl p-5 border border-white/10">
              <p className="text-slate-400 text-sm mb-2">Why This Recommendation?</p>
              <div className="bg-[#020617] rounded-2xl p-5 border border-white/10 mb-3">
                <p className="text-slate-400 text-sm">Recommended Plan</p>
                <h4 className="text-2xl font-bold mt-2">{result.optimizedPlan}</h4>
              </div>
